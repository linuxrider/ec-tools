...
The differentiation of an arbitrary function is often expressed by ??? diff for t vs. variable x? :math:`\frac{\mathrm{d}}{\mathrm{d}t} f(x)`.
Interestingly, the integration can also be expressed in a similar way:


??? limits necessary?

.. math:: \frac{\mathrm{d}^{-1}}{\mathrm{d}t^{-1}} f(x)= \int_0^t f(\tau) \mathrm{d}\tau

Therefore, these operations can be displayed in a more general way by:

??? limits necessary?

.. math:: \frac{\mathrm{d}^{v}}{\mathrm{d}t^{v}} f(x) = \int_0^t f(\tau) \mathrm{d}\tau
...
by applying another semi integration it is tranformed into a wave function (bottom right), which is equal to 
...
It can be generally expressed by the sum of the discrete function values multiplied with weights ??? w_n ? :math:`w_i` 
...
The weights can be expressed in different ways. The single weights :math:`w_i` often depend on their predecessor :math:`w_{i-1}`. 
As the factorial expression could lead to an ??? what kind? overflow, this algorithm needs to be simplified.
...
The algorithm ??? number (from Web1242) for the **R1 semi integration** is:
...
Similar to the G1 algorithm, ??? series of algorithmis? R1 is not usable for application to currents that arise from potential steps or leaps :cite:p:`oldham_electrochemical_2013`, as:
...
#. The algorithm is based on the assumption that :math:`f(t)` can be treated as an assemblage of linear segments, 
   whereas faradaic currents arising from a potential step and are ??? decidedly? non-linear with time. 
...
??? \\delta? :math:`\Delta_t` defines the constant time intervall (i.e. :math:`t_2 - t_1`) and :math:`c_1, c_2` are constant values,
which is set to :math:`c_1=8, c_2=2` by default, as recommended by Pajkossy. With these variables, the procedure of the algorithm can be described as pseudo code:

??? use same symbols as in the previous parts?

**Input** :math:`q, N, \Delta_t, c_1, c_2, I`

:math:`t_0 = \Delta_t N^{\frac{1}{2}}` 
...
def semi_integration(y, x, v=-0.5, alg="frlt", transonic_backend="pythran", d_tol=1e-5):
    r"""
    Perform a semi integration (``v`` =-0.5) or semi differentiation (``v`` =0.5)
    ...
    If steps are not equally spaced, ``d_tol`` (by default: 1 10⁻⁵) defines the maximum relational difference between individual step and the average step size.
    """
...
def gruenwald(y, delta_x, v=-0.5):
    ...
    and comparing the result with a numerical integration from scipy. First, the input data is a function with constant y:
...
def fast_riemann(y, delta_x=1, q=-0.5, c1=8, c2=2):
    ...
    based on Pajkossy et al :cite:p:`pajkossy_fast_1984_65`. Return the semiintegral R of order q for y
